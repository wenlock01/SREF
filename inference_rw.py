...
from core.networks import ImprovedAffinityNet, AdvancedAffinityNet
...
parser.add_argument('--data_dir', default=r'E:\E-images\weakly\Vaihingen\train/', type=str)
...
###############################################################################
parser.add_argument('--model_name', default='shadowAffinityNet@vaihingen@shadow_resnet50@aff_fg=0.55_bg=0.55@32_0.01epoch5', type=str)

parser.add_argument('--cam_dir', default='Vaihingen_puzzleCAM_resnet50_alpha=1.25@train@scale=1.0', type=str)
...
if __name__ == '__main__':
    ...
    experiment_name += '@rw_no'
    
    cam_dir = f'F:\FlipCAM-master-master/experiments/v-predictions/{args.cam_dir}/'
    pred_dir = create_directory(f'F:\FlipCAM-master-master/experiments/v-predictions/{experiment_name}/')

    model_path = r'F:\FlipCAM-master-master\experiments\building_vaihingen_puzzleCAM_resnet50_RGB_alpha=1.25\models/' + f'{args.model_name}.pth'
    ...
    # Network
    ###################################################################################
    path_index = PathIndex(radius=10, default_size=(256 // 4, 256 // 4))
    model = AffinityNet(args.architecture, path_index)
    # model = ImprovedAffinityNet(args.architecture, path_index)
    # model = AdvancedAffinityNet(args.architecture, path_index)
    ...
    with torch.no_grad():
        ...
        for step, (ori_image, image_id, label, gt_mask) in enumerate(dataset):
            ...
            sys.stdout.write(
                '\r# Make CAM with Random Walk [{}/{}] = {:.2f}%, ({}, rw_up={}, rw={})'.format(step + 1, length, (
                            step + 1) / length * 100, (ori_h, ori_w), rw_up.size(), rw.size()))
            sys.stdout.flush()
        print()
