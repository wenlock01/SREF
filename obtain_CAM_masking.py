# adv_climb: 计算图像的对抗性扰动，通过梯度符号调整图像像素值，并限制在数据归一化范围内。
# add_discriminative: 根据区域分数更新扩展掩码，将分数高于阈值的区域标记为1。
# 遍历数据集，计算不同尺度下的CAM（Class Activation Map）。
# 使用GradCAM生成特征图，并通过对抗训练优化CAM。
# 最终生成并保存低分辨率和高分辨率的CAM结果
...
parser.add_argument("--voc12_root", default=r'E:\E-images\weakly\Vaihingen\train/', type=str,
                    help="Path to VOC 2012 Devkit, must contain ./JPEGImages as subdirectory.")
# Dataset
parser.add_argument("--train_list", default=r"E:\E-images\weakly\Vaihingen\train/train.txt", type=str)
parser.add_argument("--val_list", default=r"E:\E-images\weakly\Vaihingen\train/val.txt", type=str)
parser.add_argument("--infer_list", default=r"E:\E-images\weakly\Vaihingen\train/train.txt", type=str,
...
# parser.add_argument("--cam_scales", default=(1.0,), help="Single-scale inferences")
...
parser.add_argument("--cam_out_dir", default="F:\FlipCAM-master-master\experiments\predictions\scale_vaihingen_obtainCAM", type=str)
...
def _work(process_id, model, dataset, args):
    ...
    with cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            size = pack['size']
            # 移除不需要的中间尺寸变量
            # strided_size = imutils.get_strided_size(size, 4)  # 不再需要
            # strided_up_size = imutils.get_strided_up_size(size, 16)  # 不再需要
            ...
            # 修改1：直接上采样到原始图像尺寸
            strided_cam = torch.sum(torch.stack(
                [F.interpolate(torch.unsqueeze(o, 0), (size[0], size[1]),  # 使用原始图像尺寸
                               mode='bilinear', align_corners=False)[0] for o in outputs_cam]), 0)

            # 修改2：不再需要额外的上采样，使用相同的CAM
            highres_cam = strided_cam.clone()  # 直接使用相同的数据
...
if __name__ == '__main__':
    ...
    state_dict = torch.load(r"E:\pycharm\FlipCAM-master-master\experiments\Vaihingen_scaleCAM_resnet50_alpha=1.25\models\Vaihingen_scaleCAM_resnet50_alpha=1.25.pth" ,weights_only=True)
