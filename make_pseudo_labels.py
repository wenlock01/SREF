# Copyright (C) 2020 * Ltd. All rights reserved.
# author : Sanghyeon Jo <josanghyeokn@gmail.com>
# 生成伪标签（pseudo labels）并保存为PNG文件

import os
import sys
import copy
import shutil
import random
import argparse
import numpy as np

import imageio

import torch
import torch.nn as nn
import torch.nn.functional as F

from torchvision import transforms
from torch.utils.tensorboard import SummaryWriter

from torch.utils.data import DataLoader

from core.puzzle_utils import *
from core.networks import *
from core.datasets import *

from tools.general.io_utils import *
from tools.general.time_utils import *
from tools.general.json_utils import *

from tools.ai.log_utils import *
from tools.ai.demo_utils import *
from tools.ai.optim_utils import *
from tools.ai.torch_utils import *
from tools.ai.evaluate_utils import *

from tools.ai.augment_utils import *
from tools.ai.randaugment import *

parser = argparse.ArgumentParser()

###############################################################################
# Dataset
###############################################################################
parser.add_argument('--seed', default=0, type=int)
parser.add_argument('--num_workers', default=4, type=int)
parser.add_argument('--data_dir', default=r'E:\E-images\weakly\inria\train/', type=str)

###############################################################################
# Inference parameters
###############################################################################
parser.add_argument('--experiment_name', default='AEANet_ablation_aff_soe@inria@shadow_resnet101@aff_fg=0.55_bg=0.55@32_0.01epoch5@edge_weight=1.0@rw', type=str)
parser.add_argument('--domain', default=r'train', type=str)
parser.add_argument('--threshold', default=0.5, type=float)
parser.add_argument('--crf_iteration', default=1, type=int)


if __name__ == '__main__':
    ###################################################################################
    # Arguments
    ###################################################################################
    args = parser.parse_args()

    cam_dir = f'F:\FlipCAM-master-master\experiments/inria-predictions/{args.experiment_name}/'
    pred_dir = create_directory(f'F:\FlipCAM-master-master\experiments/inria-predictions/{args.experiment_name}@threshold={args.threshold}@crf={args.crf_iteration}/')

    set_seed(args.seed)
    log_func = lambda string='': print(string)

    ###################################################################################
    # Transform, Dataset, DataLoader
    ###################################################################################
    dataset = VOC_Dataset_For_Making_CAM(args.data_dir, args.domain)

    #################################################################################################
    # Evaluation
    #################################################################################################
    eval_timer = Timer()

    with torch.no_grad():
        length = len(dataset)
        for step, (ori_image, image_id, label, gt_mask) in enumerate(dataset):
            png_path = pred_dir + image_id + '.png'
            if os.path.isfile(png_path):
                continue

            # ori_w, ori_h = ori_image.size
            ori_h, ori_w = ori_image.shape[1], ori_image.shape[2]
            predict_dict = np.load(cam_dir + image_id + '.npy', allow_pickle=True).item()

            keys = predict_dict['keys']
            cams = predict_dict['rw']
            # cams = predict_dict['cam']  # obtain_cam
            cams = np.pad(cams, ((1, 0), (0, 0), (0, 0)), mode='constant', constant_values=args.threshold)
            cams = np.argmax(cams, axis=0)

            # if args.crf_iteration > 0:
            #     image_np = np.asarray(ori_image)
            #     # 确保图像为(H, W, C)格式
            #     if image_np.ndim == 3 and image_np.shape[0] in [1, 3]:
            #         image_np = image_np.transpose(1, 2, 0)
            #
            #     # 确保CAM尺寸与图像一致
            #     if cams.shape != image_np.shape[:2]:
            #         from skimage.transform import resize
            #
            #         cams = resize(cams, image_np.shape[:2], order=0, preserve_range=True, anti_aliasing=False).astype(
            #             cams.dtype)
            #
            #     cams = crf_inference_label(image_np, cams, n_labels=keys.shape[0], t=args.crf_iteration)

            if args.crf_iteration > 0:
                # 将图像通道顺序从 (C, H, W) 调整为 (H, W, C)
                # image_np = np.asarray(ori_image).transpose(1, 2, 0)
                # cams = crf_inference_label(image_np, cams, n_labels=keys.shape[0], t=args.crf_iteration)
                cams = crf_inference_label(np.asarray(ori_image), cams, n_labels=keys.shape[0], t=args.crf_iteration)

            conf = keys[cams]
            # conf = keys.numpy()[cams]  # obtain_cam
            imageio.imwrite(png_path, conf.astype(np.uint8))

            sys.stdout.write('\r# Make Pseudo Labels [{}/{}] = {:.2f}%, ({}, {})'.format(step + 1, length, (step + 1) / length * 100, (ori_h, ori_w), conf.shape))
            sys.stdout.flush()
        print()

    print("python3 evaluate.py --experiment_name {} --mode png".format(args.experiment_name + f'@crf={args.crf_iteration}'))
